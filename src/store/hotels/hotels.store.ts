...
export const useHotelsStore = create<BearState>((set, get: any) => ({
  ...
  filter: async ({rating, adultsAmount, childrenAmount}) => {
    ...
    const hotelsFullInfo: Awaited<Promise<HotelWithDetails[]>> = await Promise.all(hotelsWithRating.map(getHotelFullInfo))
    const filteredHotels = hotelsFullInfo.reduce((result: HotelWithDetails[], hotelFullInfo: HotelWithDetails) => {
      const availableRooms = hotelFullInfo.rooms.filter(({ occupancy }) => 
